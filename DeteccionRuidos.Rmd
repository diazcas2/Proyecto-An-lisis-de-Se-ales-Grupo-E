...
```{r}
# Esto se puede eliminar en un futuro si se hace un bucle que pase todos los audios a las funciones correspondientes
o1<-readWave('Audioswav_prueba/Oscar a1.wav')
a1<-readWave('Audioswav_prueba/aza a1.wav')
plot(o1)
```
...
```{r}
noise_audio <- function(audio, win_time = 0.02, overlap = 50, umbral_zcr = 0.05, umbral_ste = 0.05, plot_result = T){
...
## Pruebas para eliminar ruido y silencios del audio
```{r}
...
```{r}
audio_limpio <- limpiar_audio(o1, resultado_analisis)

# Comparación del audio original y el procesado
par(mfrow=c(2,1))
oscillo(o1, title = "Original")
oscillo(audio_limpio, title = "Procesado")

writeWave(audio_limpio, "Audioswav/Oscar_a1_limpio.wav")
```
