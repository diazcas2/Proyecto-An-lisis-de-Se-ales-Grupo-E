...
extract_feat_from_wav <- function(
  ...
  # Limpieza MFCC y derivadas
  mfcc[!is.finite(mfcc)] <- NA_real_
  if (all(is.na(mfcc))) return(NULL)

  d1 <- delta_simple(mfcc); d1[!is.finite(d1)] <- NA_real_
  d2 <- delta_simple(d1);   d2[!is.finite(d2)] <- NA_real_
  ...
  names(feat_mfcc) <- nm_mfcc

  # --- NUEVO: centroide por ventanas + stats (misma wintime/hoptime) ---
  cent_feat <- centroid_stats_from_wave(
    w,
    wintime = wintime,
    hoptime = hoptime,
    energy_floor = centroid_energy_floor
  )
  # (cent_feat ya viene con nombres: cent_mean, cent_sd, ...)

  # Unir todo
  feat <- c(feat_mfcc, cent_feat)
...
# Centroide para UN frame (vector) a fs
centroid_frame <- function(x, fs) {
  x <- x - mean(x)
  N <- length(x)
...
# Centroide por frames (misma vent/hop que MFCC) y estadísticos agregados
centroid_stats_from_wave <- function(w,
                                     wintime = 0.025,
                                     hoptime = 0.010,
                                     energy_floor = 1e-8) {
  w <- to_mono(w)
  ...
  nwin <- max(16, round(wintime * fs))
  hop  <- max(1,  round(hoptime * fs))

  if (length(x) < nwin) {
    return(c(
      cent_mean = NA_real_, cent_sd = NA_real_,
      cent_p10 = NA_real_, cent_p50 = NA_real_, cent_p90 = NA_real_,
      cent_iqr = NA_real_,
      cent_d1_mean = NA_real_, cent_d1_sd = NA_real_
    ))
  }

  # Ventana Hann para estabilizar el espectro por frame
  win <- 0.5 - 0.5 * cos(2*pi*(0:(nwin-1))/(nwin-1))

  idx_starts <- seq(1, length(x) - nwin + 1, by = hop)
  cents <- numeric(0)
  ...
  for (i in idx_starts) {
    ...
    # Gate para evitar silencios (muy importante para no sesgar estadísticos)
    if (!is.finite(e) || e < energy_floor) next

    frame <- frame * win
    cval <- centroid_frame(frame, fs)
    if (is.finite(cval)) cents <- c(cents, cval)
  }
  ...
  d1 <- if (length(cents) >= 2) diff(cents) else NA_real_

  c(
    cent_mean = mean(cents, na.rm = TRUE),
    cent_sd   = sd(cents,   na.rm = TRUE),
    cent_p10  = as.numeric(quantile(cents, 0.10, na.rm = TRUE)),
    cent_p50  = as.numeric(quantile(cents, 0.50, na.rm = TRUE)),
    cent_p90  = as.numeric(quantile(cents, 0.90, na.rm = TRUE)),
    cent_iqr  = IQR(cents, na.rm = TRUE),
    cent_d1_mean = mean(d1, na.rm = TRUE),
    cent_d1_sd   = sd(d1,   na.rm = TRUE)
  )
}
...
dir.exists("for-norm/training/real")
dir.exists("for-norm/training/fake")

real_files <- list.files("for-norm/training/real", pattern="\\.(wav|WAV)$", full.names=TRUE, recursive=TRUE)
fake_files <- list.files("for-norm/training/fake", pattern="\\.(wav|WAV)$", full.names=TRUE, recursive=TRUE)
...
df <- build_feat_dataframe(
  real_dir = "datos_nuestros/test_con_mod_int/train_real",
  ia_dir   = "datos_nuestros/test_con_mod_int/train_fake",
  target_sr = 16000,
  dither = FALSE
)
# str(df)
...
```
```{r}
saveRDS(df, "df_nuestro.rds")
```

```{r}
df<-readRDS("df_nuestro.rds")
```
...
```{r}
saveRDS(train_df, "train_df_nuestro.rds")
```
...
#saveRDS(svm_fit, "entrenamiento_MFCC, modelo_svm_feat_nuestro.rds")
```
```{r}
saveRDS(svm_fit, "variables_entrenamiento_nuestro/svm_fit_nuestro.rds")
```
...
df_test <- build_feat_dataframe(
  real_dir = "datos_nuestros/real_total",
  ia_dir   = "datos_nuestros/fake_total",
  target_sr = 16000,
  dither = FALSE
)
```
...
```{r}
saveRDS(df_test, "variables_entrenamiento_nuestro_total/df_test_total.rds")
```
...
```{r}
saveRDS(df_test_ml, "variables_entrenamiento_nuestro_total/df_test_ml_nuestro_total.rds")
saveRDS(df_test_ok, "variables_entrenamiento_nuestro_total/df_test_ok_nuestro_total.rds")
```
```{r}
