...
```{r}
svm_feat_internet<-readRDS("variables_entrenamiento_internet/svm_fit.rds")
df_test_internet_ml<-readRDS("variables_entrenamiento_internet/df_test_ml.rds")
df_test_internet_ok<-readRDS("variables_entrenamiento_internet/df_test_ok.rds")
```
...
```{r}
df_test_nuestro_ml_total<-readRDS("variables_entrenamiento_nuestro_total/df_test_ml_nuestro_total.rds")
df_test_nuestro_ok_total<-readRDS("variables_entrenamiento_nuestro_total/df_test_ok_nuestro_total.rds")
```
...
```{r}
# 3) Predicción
p <- predict(svm_feat_internet, newdata = df_test_nuestro_ml_total, type = "prob")[, "ia"]
pred <- predict(svm_feat_internet, newdata = df_test_nuestro_ml_total, type = "raw")

# 4) Métricas
confusionMatrix(pred, df_test_nuestro_ml_total$label)

roc_obj <- roc(response = df_test_nuestro_ml_total$label, predictor = p, levels = c("real","ia"))
auc(roc_obj)
plot(roc_obj)

# 5) Guardar resultados por archivo
results <- data.frame(
  file = df_test_nuestro_ok_total$file,
  label = df_test_nuestro_ok_total$label,
  prob_ia = p,
  pred = pred,
  stringsAsFactors = FALSE
)
...
```{r}
roc_obj <- roc(df_test_nuestro_ml_total$label, p, levels = c("real","ia"))
coords(roc_obj, "best", best.method = "youden",
       ret = c("threshold","sensitivity","specificity"))
```
...
pred_thr <- factor(ifelse(p >= thr, "ia", "real"), levels = c("real","ia"))
cm_thr <- confusionMatrix(pred_thr, df_test_nuestro_ml_total$label)
cm_thr
cm_thr$table
...
```{r}
svm_feat_nuestro<-readRDS("variables_entrenamiento_nuestro/svm_fit_nuestro.rds")
df_test_nuestro_ml<-readRDS("variables_entrenamiento_nuestro/df_test_ml_nuestro.rds")
df_test_nuestro_ok<-readRDS("variables_entrenamiento_nuestro/df_test_ok_nuestro.rds")
```
