---
title: "Análisis y clasificación de voz"
author:
...
El código fuente completo del proyecto, junto con los datos y scripts necesarios para reproducir los resultados, están disponibles en GitHub:
https://github.com/diazcas2/Proyecto-An-lisis-de-Se-ales-Grupo-E.git

## 2. PREPROCESADO

### 2.1. Conversión y lectura de audios
...
### 2.2. Detección y eliminación de ruido y silencios
...
De acuerdo con la literatura consultada, los valores típicos de frecuencia fundamental en voz humana se sitúan aproximadamente en el rango comprendido entre 85 y 260 Hz, presentando diferencias claras entre hombres y mujeres \citep{rabiner1978digital, basu2020pitch}. 
...
### 3.2. Coeficientes cepstrales en escala Mel (MFCC) y representación de la señal

Como parte de la extracción de características espectrales, se calcularon los coeficientes cepstrales en escala Mel (MFCC), ampliamente utilizados en tareas de análisis de voz por su capacidad de capturar información relacionada con el timbre y la envolvente espectral. Los MFCC son características que resumen el timbre de la voz a partir de su espectro, usando una escala de frecuencias similar a la percepción humana (escala Mel). 

En el código, para calcular estos coeficientes, se usa la función `melfcc()` de la libreria `tuneR`. Esta función divide la señal en ventanas temporales solapadas de duración 25 ms con un salto de 10 ms. Para cada ventana se obtiene el espectro (vía FFT), se proyecta sobre un banco de filtros Mel con 40 bandas, se aplica compresión logarítmica y posteriormente una transformación tipo DCT para obtener un número reducido de coeficientes. En este caso se conservan 12 coeficientes por ventana.

Además se calculan $\Delta$ y $\Delta\Delta$ que son las derivadas temporales de los MFCC: $\Delta$ mide cómo cambian de un frame al siguiente (dinámica) y $\Delta\Delta$ mide el cambio de ese cambio.



### 3.3. Centroide espectral
...
## 4. CONSTRUCCIÓN DEL CONJUNTO DE DATOS

### 4.1. Conjunto de datos general
...
### 4.2. Conjunto de datos para detección de IA: MFCC
...
Analizando más profundamente el centroide espectral global, se observa una diferencia clara entre las voces humanas y las generadas por inteligencia artificial, tanto en el grupo masculino como en el femenino. En ambos casos, las voces generadas por IA presentan valores de centroide más elevados, lo que indica una mayor concentración de energía en frecuencias altas. Esta diferencia es especialmente marcada en las voces femeninas, donde el solapamiento entre clases es reducido. En el caso de las voces masculinas, aunque la tendencia se mantiene, se aprecia un mayor solapamiento entre ambas distribuciones.
...
Inicialmente, se ajustó un modelo de regresión logística multivariante que incluía todas las características disponibles en el conjunto de datos: duración, ZCR, energía RMS, Period Pitch, centroide, así como las variables categóricas de origen y acento. Este primer modelo tiene como objetivo evaluar la contribución conjunta de todas las variables y explorar posibles relaciones entre ellas y la variable respuesta.
...
En conjunto, estos resultados refuerzan que el pitch y el centroide son los descriptores más relevantes para la clasificación del sexo, pero que la superposición natural de frecuencias humanas introduce un margen de error inevitable, explicando los puntos intermedios en la gráfica de predicciones.

### 6.2. Clasificación del origen (IA vs Humano): Coeficientes cepstrales con escala Mel
...
- Suele funcionar muy bien cuando hay un número medio de características (ni muy pocas ni miles)\cite{mathworks_fitcsvm}
- Es un baseline clásico y robusto en tareas de voz y audio cuando se trabaja con MFCC y estadísticas agregadas.\cite{temko2006classification}
...
\[
\begin{array}{c|cc}
\textbf{Predicción}  & \textbf{real} & \textbf{ia} \\
\hline
\textbf{real} & 2063 & 106 \\
\textbf{ia}   & 201  & 2264 \\
\end{array}
...
### 6.3. Clasificación del acento

Además de los clasficadores anteriores, se intentó abordar la identificación del acento de la voz, considerando tres categorías: español neutro, andaluz y argentino. 
...
Tras analizar los resultados, vimos que, aunque el modelo consigue ajustar los datos ($R^2_{\text{McFadden}}~$ = 0.5296), los resultados no pueden considerarse fiables. El número de muestras por acento no es equilibrado: el grupo cuenta con tres personas con acento español neutro (dos hombres y una mujer), dos con acento andaluz (hombre y mujer) y una sola con acento argentino (mujer). La variabilidad interna de cada clase es muy limitada al usar varios audios de las mismas voces. Esto hace que, en la práctica, el modelo tiende a aprender rasgos de cada persona más que del acento en sí.
...
El desarrollo del proyecto ha permitido demostrar que es posible analizar señales de voz y extraer de ellas información acústica suficiente para abordar con éxito distintas tareas de clasificación. 

En particular, se han obtenido buenos resultados en la identificación del sexo del hablante y en la diferenciación entre voces humanas y voces generadas por IA, lo que confirma que la combinación de técnicas de preprocesado, extracción de características y aprendizaje automático constituye una herramienta eficaz para el análisis de voz en entornos reales.

Por el contrario, la clasificación del acento no ha alcanzado el nivel de precisión esperado. Los resultados apuntan a que este problema requiere bases de datos considerablemente más amplias y diversas, ya que la variabilidad entre los acentos y el solapamiento entre categorías dificultan la generalización de los modelos entrenados. Lejos de interpretarse como una limitación negativa, este resultado motiva a una investigación futura donde la relevancia del tamaño muestral y de la representatividad de los datos ocupan una gran importancia.

En conjunto, el trabajo ha permitido alcanzar la mayor parte de los objetivos planteados y entender mejor qué descriptores son realmente informativos según cada clasificador. Además, muestra que técnicas relativamente sencillas pueden abordar problemas actuales como la detección de voces sintéticas, con posibles aplicaciones en seguridad y verificación de identidad.

Como continuación natural del proyecto, sería interesante ampliar la base de datos, incorporar nuevos descriptores o probar modelos más avanzados. Los resultados obtenidos sirven así como un buen punto de partida para seguir profundizando en el análisis automático de la voz.