...
noise_audio <- function(audio, win_time = 0.02, overlap = 50, umbral_zcr = 0.5, umbral_ste = 0.02, plot_result = T){
...
De acuerdo con la literatura consultada, los valores típicos de frecuencia fundamental en voz humana se sitúan aproximadamente en el rango comprendido entre 85 y 260 Hz, presentando diferencias claras entre hombres y mujeres ([@rabiner1978digital] y [@basu2020pitch]). 
...
Con el objetivo de caracterizar el contenido espectral de las señales de voz, se implementó una función para el cálculo del centroide espectral global de cada audio. Este descriptor se define como la frecuencia media ponderada por la energía del espectro y proporciona una medida resumen de la distribución espectral de la señal. Previamente al cálculo, se elimina el componente de continua de la señal con el fin de evitar un sesgo hacia bajas frecuencias. El centroide se obtiene a partir del espectro de potencia calculado mediante la transformada rápida de Fourier, considerando únicamente las componentes de frecuencia positivas. De este modo, cada señal queda representada por un único valor que resume su contenido espectral global.
...
## Construcción de los conjuntos de datos para detección de IA: Centroide espectral

También se generaron dos conjuntos de datos específicos basados en el centroide espectral. El primero de ellos contiene, para cada señal, las etiquetas de sexo, origen y acento junto con el valor del centroide espectral global, calculado sobre la totalidad del audio. El segundo conjunto de datos incluye las mismas etiquetas y una representación temporal del centroide espectral, obtenida mediante la segmentación de cada señal en doce partes, junto con una medida agregada de su variabilidad, calculada como la desviación típica de los doce centroides.

Esto lo utilizaremos posteriormente para ver como funciona esta característica a la hora de clasificar IA-humano.

```{r, echo = FALSE}
# DataFrame con centroide global

carpeta_audios <- "Audios_finales_limp"

df_centroid_global <- voces_df[, c("señal", "sexo", "origen", "acento")]
df_centroid_global$centroid_global <- sapply(df_centroid_global$señal, function(nombre) {
  w <- readWave(file.path(carpeta_audios, nombre))
  spectral_centroid(w)
})

# DataFrame con centroide segmentado en 12 partes

centroid12_mat <- t(sapply(voces_df$señal, function(nombre) {
  w <- readWave(file.path(carpeta_audios, nombre))
  spectral_centroid_12(w, n_parts = 12)
}))

colnames(centroid12_mat) <- paste0("centroid_", 1:12)

df_centroid_12 <- cbind(
  voces_df[, c("señal", "sexo", "origen", "acento")],
  as.data.frame(centroid12_mat)
)

df_centroid_12$centroid_sd12 <- apply(centroid12_mat, 1, sd, na.rm = TRUE)
...
```{r, echo = FALSE}
ggplot(df_centroid_global, aes(x = origen, y = centroid_global, fill = origen)) +
  geom_boxplot() +
  facet_wrap(~ sexo) +
...
Las tres categorías (AN: andaluz, AR: argentino, N: neutro) presentan rangos de ZCR parcialmente solapados, lo que indica que no hay una separación clara solo con esta característica.

El acento neutro (N) muestra una mediana de ZCR ligeramente más alta y una dispersión mayor, mientras que AN y AR parecen algo más compactos y con medianas algo inferiores.
...
La energía RMS resume la intensidad media de la señal y puede reflejar diferencias en el patrón de acentuación o en la “fuerza” con la que se articula.

La duración del audio captura diferencias en el tempo o velocidad de habla, ya que algunos acentos tienden a ser más rápidos o más pausados.

El period pitch, inversamente relacionado con la frecuencia fundamental, recoge variaciones en la entonación y altura de la voz, otro rasgo típicamente distintivo entre acentos.
...
En total se entrenaron 2 modelos, con 2 datasets diferentes: un dataset extraido de internet (enlace al dataset: \href{https://bil.eecs.yorku.ca/datasets/}{dataset for-norm}) y otro dataset formado por audios nuestros y generados con IA por nosotros.
...
A continuación hemos realizado dos funciones, una para que realice un random forest del centroide espectral y otra de los centroides espectrales segmentados y su desviación típica. 

```{r, echo = FALSE}
rf_by_sex <- function(df, sex_value, seed = 123, train_prop = 0.7) {
  d <- subset(df, sexo == sex_value)
  d <- d[is.finite(d$centroid_global) & !is.na(d$origen), ]
  ...
  rf <- randomForest(origen ~ centroid_global, data = train)
...
```{r, echo = FALSE}
rf_M <- rf_by_sex(df_centroid_global, "M")
rf_F <- rf_by_sex(df_centroid_global, "F")
...
```{r, echo = FALSE}
rf12sd_M <- rf_by_sex_centroid12_sd(df_centroid_12, "M")
rf12sd_F <- rf_by_sex_centroid12_sd(df_centroid_12, "F")
...
Esto puede deberse a que el diseño del dataset introduce una estructura muy repetitiva: mismas frases, solo una variedad de 5 personas (por lo que se repiten mucho las mismas voces) y misma cadena de grabación y limpieza, además de realizar el mismo procedimiento de generación IA usando también las mismas frases. Esto puede provocar que el modelo no esté aprendiendo “humanidad vs IA” en un sentido general, sino artefactos muy específicos de nuestro contexto.
