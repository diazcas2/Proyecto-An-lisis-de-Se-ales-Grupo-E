...
\subsection{Clasificación del sexo a partir del pitch}

Con el objetivo de diferenciar entre voces masculinas y femeninas, se utilizó el **period pitch** de la señal de voz como principal descriptor. Para su estimación, se implementó una función basada en el análisis de autocorrelación, a partir de la cual se obtuvo el período fundamental y, de forma inversa, el valor del pitch.

Inicialmente, se realizó una clasificación directa mediante umbrales sobre el valor del pitch, considerando rangos típicos de frecuencia fundamental para voces masculinas y femeninas. Esta aproximación permitió obtener una primera predicción del sexo asociada a cada audio.
...
Posteriormente, se planteó un enfoque estadístico mediante un modelo de regresión logística binaria, donde la variable respuesta corresponde al sexo de la voz y las variables explicativas incluyen distintas características acústicas extraídas previamente. Debido al tamaño reducido del conjunto de datos, se empleó validación cruzada \textit{k-fold} con $k=5$ para evaluar la capacidad de generalización de los modelos.
...
Se comparó un modelo completo, que incorpora múltiples descriptores acústicos, con un modelo simplificado que utiliza únicamente el \textit{period pitch} como variable predictora. Los resultados mostraron que el modelo simple presentaba un menor error de clasificación, lo que indica que el pitch es un descriptor altamente informativo para la tarea de clasificación de género en este contexto.

Finalmente, el modelo seleccionado fue entrenado utilizando una partición de los datos en conjuntos de entrenamiento y prueba, obteniéndose una elevada correlación entre las predicciones y las etiquetas reales, lo que confirma la eficacia del enfoque propuesto.
...
\subsection{Detección de voz generada por inteligencia artificial}
