...
Comenzamos haciendo un estudio de las características acústicas para ver si alguna podía, por sí sola, ayudar a distinguir entre acentos. Nos centramos en analizaren detalle la tasa de cruces por cero (ZCR) dado que esta es especialmente interesante porque está muy relacionada con el contenido de altas frecuencias y con la presencia de consonantes fricativas o sonidos sin sonoridad, que pueden variar entre acentos, por ejemplo, por mayor aspiración de ciertas consonantes, articulación más “suave” o más “marcada”, etc.

Por eso se representó la distribución del ZCR por acento mediante un diagrama de cajas. 
En el gráfico se observa que:

Las tres categorías (AN: andaluz, AR: argentino, N: neutro) presentan rangos de ZCR parcialmente solapados, lo que ya sugiere que no hay una separación clara solo con esta característica.
...
En conjunto, el boxplot indica que el ZCR contiene cierta información sobre el acento (las distribuciones no son idénticas), pero la superposición tan grande entre cajas apunta a que ZCR por sí solo no es suficiente para discriminar con fiabilidad los acentos. Esto justifica la combinación de ZCR con otras características en un modelo multinomial.
...
Para esta tarea se planteó el problema como una clasificación multiclase, por lo que realizamos un modelo de regresión logística multinomial, utilizando como variables predictoras descriptores acústicos de bajo nivel, concretamente la tasa de cruces por cero (ZCR), la energía RMS, la duración del audio y el pitch period. Estas características se seleccionaron por su relación con aspectos articulatorios y prosódicos que pueden variar entre acentos, los cuales se detallan a continuación:
...
Estas variables, junto con el ZCR, permiten modelar tanto aspectos segmentales (contenido en altas frecuencias, presencia de fricativas, etc.) como patrones suprasegmentales (ritmo, intensidad e entonación), que son precisamente los elementos en los que suelen diferir los acentos. Por ello se integraron conjuntamente en el modelo de regresión logística multinomial.
...
Por último, se ajustó el modelo multinomial y se evaluó su capacidad explicativa mediante el pseudo-$R^2$ de McFadden.
En modelos de regresión logística (incluida la regresión multinomial) el $R^2$ clásico de regresión lineal no es adecuado, porque no existe una descomposición de la varianza análoga, y por ello se recurre a índices basados en la verosimilitud. El pseudo-$R^2$ de McFadden es uno de los más utilizados porque compara de forma directa el modelo con predictores frente a un modelo nulo, y se define como
