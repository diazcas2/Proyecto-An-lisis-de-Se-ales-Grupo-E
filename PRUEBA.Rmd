...
#Creamos el modelo para predecir el acento
...
#Hacemos un gráfico para ver cómo varía el ZCR según el acento
...
```{r}
# listar todos los wav de la carpeta
files <- list.files(
  "Audios_finales_limp",
  pattern = "\\.wav$",
  full.names = TRUE
...
voces_df$centroid <- sapply(voces_df$señal, function(nombre) {
  w <- readWave(file.path("Audios_finales_limp", nombre))
  spectral_centroid(w)
})
```
...
rf_by_sex <- function(df, sex_value, seed = 123, train_prop = 0.7) {
  ...
  # nos quedamos con filas válidas
  d <- d[is.finite(d$centroid) & !is.na(d$origen), ]
  ...
  rf <- randomForest(origen ~ centroid, data = train)
...
# SPECTRAL CENTROID PARTIDO EN 12 CON SD
...
ggplot(voces_df, aes(x = origen, y = centroid_diff_mean, fill = origen)) +
  geom_boxplot(alpha = 0.7) +
  facet_wrap(~ sexo) +
  labs(
    title = "Variabilidad temporal del centroide espectral por sexo",
    x = "Origen de la voz",
    y = "Desviación típica de las diferencias entre centroides"
  ) +
  theme_minimal() +
